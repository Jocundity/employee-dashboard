export async function getLocation(city) {
    ...
    if (data.results && data.results.length > 0) {
        ...
        for (let i =0; i < data.results.length; i++)    {
            cities.push({id: data.results[i].id,
...
export async function getWeather(latitude, longitude) {
    const WEATHER_API_URL = `https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&current=temperature_2m,wind_speed_10m,precipitation&daily=precipitation_sum,temperature_2m_max,temperature_2m_min&forecast_days=7`
    ...
    return {
        ...
        forecast: data.daily.time.map((date, index) => ({
            date: new Date(date).toLocaleDateString('en-GB', {weekday: 'short', day: 'numeric', month: 'numeric'}),
